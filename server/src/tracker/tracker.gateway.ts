...
export class TrackerGateway {
    ...
    protected sendToAllClients(msg: RldNodeMessage) {
      for (const [id, client] of this.activeClients) {
        this.sendToClient(client, msg);
      }
    }
  
    protected sendToClient(client: Ws, msg: RldNodeMessage) {
      const buffer = {event: 'msg', data: JSON.stringify(RldNodeMessage.toJSON(msg))};
